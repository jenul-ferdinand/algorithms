...
class UkkonenSuffixLinks:
    ...
    def _build(self) -> None:
        ...
        for i in range(self.n):
            ...
            self._apply_extension(extension_point, j=0, i=i)
            ...
            for j in range(1, i + 1):
                ...
                self._apply_extension(extension_point, j, i)
    ...
    def _walk_down(
        ...
        while k < i:
            ...
            while e <= edge.end and k < i:
                assert S[k] == S[e], (
                    "walk-down invariant: path must already exist in tree"
                )
                e += 1
                k += 1
            ...
            if k == i:
                if e == edge.end + 1:
                    ...
                    if edge.child.is_leaf:
                        # at leaf, remainder is path from parent to leaf
                        state = ActiveState(
                            node=curr, remainder_start=edge_path_start
                        )
                    else:
                        # at internal, remainder empty
                        state = ActiveState(node=edge.child, remainder_start=i)
                    return extension_point, state
                else:
                    # mid-edge
                    extension_point = MidEdge(edge=edge, chars_into_edge=e - edge.start)
                    # remainder is path from parent to mid-edge
                    state = ActiveState(
                        node=curr, remainder_start=edge_path_start
                    )
                    return extension_point, state
