...
- [x] Definition 6: Rule 1 extension
...
- [x] Definition 7: Rule 2 case 1 (regular)
...
- [x] Definition 9: Rule 2 case 2 (alternate / split)
...
- [x] Definition 8: Rule 3 extension (do nothing)
...
- [x] Worked example: build I_1 through I_4 for `abac` by hand
...
- [x] Lab notes-Q3: append `$` to `abac` and run one more phase to obtain the regular suffix tree for `abac`.
...
- [x] Impl: naive Ukkonen (lab04 step 2)
    - File target: `src/03_NaiveImplicitSuffixTree.py`.
...
- [x] Definition 11: active node and remainder
...
- [x] The skip-count trick (Section 3.4)
...
- [x] Step 1: Node and Edge with (startIndex, endIndex) labels (Section 1, Section 6).
- [x] Step 2: naive O(n^2) suffix tree by direct insertion (Section 1).
- [x] Step 3: naive Ukkonen with all four rules, root-restart, character traversal (Section 3).
- [x] Step 4: skip-count traversal from root (Section 5).
- [x] Step 5: showstopper rule (Section 7).
- [x] Step 6: globalEnd + last_j (Section 8).
- [x] Step 7: suffix link creation on Rule 2 case 2 (Section 10).
- [x] Step 8: active-node optimisation, no more root restart (Section 10).
- [x] Step 9: regression tests against the naive suffix tree on a corpus including `abac$`, `abacabad$`, `abaaba$`, `mississippi$`, random strings.
