...
- [x] Heaps as the canonical implementation
...
- [x] Why this matters for Fibonacci heaps
...
- [x] Why circular DLLs everywhere
...
- [x] Consolidation via auxiliary array A
...
- [x] Edge cases during consolidation
...
- [x] Tree-merge primitive (notes pg 16)
...
- [x] O(1) amortised, O(log N) worst case
...
- [x] Reduction to decrease-key + extract-min
...
- [x] Why -infinity (or any small enough sentinel) works
...
- [x] The two competing goals (notes pg 21)
...
- [x] Why "after two" and not "after one"
    - The reason we cut after 2 (and not after just 1) children is to limit the frequency of the cascading cuts as required by property 2.
...
- [x] Lab Q6: prove sum from i=0 to n of F_i = F_{n+2} - 1 by induction
...
- [x] Lab Q7: prove F_{n+2} >= phi^n by induction, where phi = (1 + sqrt 5)/2
    - Base: F_2 = 1 >= phi^0 = 1.
...
- [x] Theorem 1: size(x) >= F_{degree(x) + 2} for any node x
...
- [x] Lab Q4: worst-case time complexities of insert and extract-min
...
- [x] Lab Q2: insert {3, 8, 10, 11, 1, 12, 2, 17}, then extract-min
...
- [x] Lab Q3: extract-min on the given heap
...
- [x] Lab Q5: sequence of decrease-key operations + extract-min
...
- [x] Step 4: decrease-key with cascading cuts.
...
- [x] Step 6 (Lab Impl Q1): full test battery.
